...
def verify_merkle_inclusion(entry) -> None:
    """Verify the Merkle Inclusion Proof for a given Rekor entry"""

    # Extract needed data from entry
    ...
    # ourselves.
    leaf_hash: bytes = _hash_leaf(base64.b64decode(entry_body))
    ...
    intermediate_result: bytes = _chain_inner(
        leaf_hash, hashes[:inner], log_index
    )

    calc_hash: str = _chain_border_right(
        intermediate_result, hashes[inner:]
    ).hex()

    if calc_hash != root_hash:
        raise InvalidInclusionProofError(
            f"Inclusion proof contains invalid root hash: expected {root_hash}, calculated "
            f"{calc_hash}"
        )
